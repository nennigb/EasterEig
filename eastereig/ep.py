...
The derivative of \(g\) and \(h\) at \(\nu_0\) can be directly obtained from the derivative of the selected eigenvalues. Applying Liebnitz' rule for product derivation yields
$$
\begin{align}
...
class EP:
    ...
    ----------
    vp1, vp2: Eig
        provides the pair of Eig object you want to merge
    ...
    def __repr__(self):
        ...
        try:
            ...
            EP_loc=[]
            Err=[]
        return "Instance of EP class\n#{} EP found merging between {} and {}\n  > nu0 : {} \n  > (N-1)-Err : {}\n  > EP list :{}".format(len(EP_loc),self._vp1.lda,self._vp2.lda, self.nu0,  Err, EP_loc)
    ...
    def Pmatrix(z0,N):
        ...
        and Taylor expansions of the function g(\\nu) \n
    ...
    def solveOddPower(xi,c,N):
        r""" Compute odd terms of the Puiseux expansion using iterative solver of the Non-Linear system
    ...
    def _dh(self):
        ...
        """       
        dh = EP.dlda2dh(self._vp1.dlda,self._vp2.dlda)
        # store
        self.dh = dh
    ...
    def _dg(self):
        ...
        self.dg = np.array(self._vp1.dlda) + np.array(self._vp2.dlda)
        # store also Taylor coeff
...
#                print('> EP Error vs N and N-1 = %f\n' % (abs(EP_locN1-EP_loc)/abs(EP_loc)))
    ...
    def getPuiseux(self, index=0):
        ...
        self.a[index] = np.concatenate([(ae0[i],ao[i]) for i in range(len(ae0))]) ### Coeff of the Puiseux serie of the first eigenvalue

        return self.a


    def plotZeros(self,index=0,Title='empty',Couleur='k',variable='\\nu',fig=-1):
        """
        ...
        plt.scatter(self.nu0.real,nu0.imag,marker='x',s=80,color=Couleur)
        # Th_N-1 roots
        zeros1 = self._roots(tronc=1)
        plt.scatter(zeros1.real[0::],zeros1.imag[0::],marker='o',facecolors='none', edgecolors=Couleur,s=80,color=Couleur)
